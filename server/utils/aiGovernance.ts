...
export function getAiAuditLog(userId?: string): AiAuditLogEntry[] {
  const now = new Date();
  const validLogs = auditLog.filter(
    (entry) => (!entry.expiresAt || entry.expiresAt > now) && (!userId || entry.userId === userId)
  );
  return [...validLogs];
}
...
const REDACTION_PATTERNS: RedactionPattern[] = [
  // Financial identifiers
  {
    name: "iban",
    regex:
      /\b[A-Z]{2}\d{2}\s?[A-Z0-9]{4}\s?[A-Z0-9]{4}\s?[A-Z0-9]{4}\s?[A-Z0-9]{4}\s?[A-Z0-9]{0,2}\b/gi,
  ...
  {
    name: "phone_uk",
    regex: /\b(?:\+44|0)\s?(?:7\d{3}|\d{4})\s?\d{6}\b/g,
  ...
  {
    name: "titled_name",
    regex: /\b(?:Mr|Mrs|Ms|Miss|Dr|Prof|Sir|Dame)\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}\b/g,
  ...
  // Addresses
  {
    ...
    replacement: "[REDACTED_POSTCODE]",
  },
  {
    name: "address_line",
    regex:
      /\b\d{1,5}\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+(?:Street|St|Road|Rd|Avenue|Ave|Lane|Ln|Drive|Dr|Court|Ct|Close|Cl|Way|Place|Pl|Crescent|Cres|Gardens|Gdns|Terrace|Ter)\b/gi,
  ...
  {
    name: "company_number",
    regex: /\b(?:SC|NI|OC|SO|NC|NL|R)?\d{6,8}\b/g,
    replacement: "[REDACTED_COMPANY_REG]",
  },
...
export function chunkCsvByRows(csvData: string, maxRowsPerChunk: number = 500): string[] {
  const lines = csvData.trim().split("\n");
  ...
  try {
    ...
    if (!aiDataConsent) {
      return {
        hasConsent: false,
        message:
          "AI data processing consent required. Please enable AI features in Settings to continue.",
      };
    }
...
export async function requirePremiumAndConsent(
  ...
  if (user.subscriptionTier !== "premium") {
    return {
      authorized: false,
      error: "Premium subscription required for Credit Underwriting",
      code: 403,
    };
  }
