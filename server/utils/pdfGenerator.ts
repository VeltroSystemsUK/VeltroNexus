...
function validateFinancials(ddData: any): IntegrityResult {
  ...
  const repayment = ddData?.loanCalculator?.monthlyPayment || 1;
...
function renderMeasuredTextCard(params: MeasuredTextCardParams): { y: number; pageNumber: number } {
  const { doc, title, text, accentColor = "#2563EB", fillColor = "#f8f9fa" } = params;
  let { y, pageNumber: pn } = params;
  const PAGE_HEIGHT_LOCAL = 841.89;
  const MARGIN_LOCAL = 50;
  const CONTENT_WIDTH_LOCAL = 495.28;
  
  if (y > PAGE_HEIGHT_LOCAL - 150) {
    doc.addPage();
    pn++;
    y = MARGIN_LOCAL + 20;
  }
  
  const textHeight = doc.heightOfString(text, { width: CONTENT_WIDTH_LOCAL - 30, fontSize: 9 });
  const boxHeight = Math.max(textHeight + 40, 60);
  
  doc.rect(MARGIN_LOCAL, y, CONTENT_WIDTH_LOCAL, boxHeight).fillAndStroke(fillColor, "#dfe6e9");
  doc.rect(MARGIN_LOCAL, y, 5, boxHeight).fill(accentColor);
  
  doc.font("Helvetica-Bold").fontSize(10).fillColor(accentColor);
  doc.text(title, MARGIN_LOCAL + 15, y + 10);
  
  doc.font("Helvetica").fontSize(9).fillColor("#333333");
  doc.text(text, MARGIN_LOCAL + 15, y + 28, { width: CONTENT_WIDTH_LOCAL - 30 });
