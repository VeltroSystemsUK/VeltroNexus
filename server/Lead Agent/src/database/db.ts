/**
 * database/db.ts
 * SQLite persistence using better-sqlite3 (synchronous, fast, zero-config).
 * Agents never touch this layer directly — all access goes through api.ts.
 */
...
export function initDb(): void {
  ...
  // Migrations for Companies House
  ...
  return {
    ...
    lenderNames: (() => {
      try {
        ...
        console.error("Failed to parse lender_names:", row.lender_names);
        return [];
      }
    })(),
  };
...
export function upsertBusiness(business: Business): Business {
  ...
  if (existing) {
    // Update enrichment fields only — don't overwrite scraped data
    db.prepare(`
...
export function getBusinessesForExport(filters: SearchFilters): Business[] {
  ...
  if (filters.operationalOnly) {
    conditions.push("status = 'OPERATIONAL'");
  }
...
export function getRunStats(searchQuery?: string): {
  ...
  const rows = db.prepare(`SELECT * FROM businesses ${where}`).all(...params) as DbRow[];
