...
| **motion** | `schema` (WebAnimationIntegrationSchema v1), `preview` live/still/reduced, optional `capturedAssetId` / `seed` / mask / stroke, optional caption `text`/`text2` (double-click or Enter to edit; wins over sibling Hook 1), optional `widget` extras (texture, drop shape, drip motion, liquid physics). Sim blits an offscreen bitmap. 90 house presets (default Ledger Current). |
...
- **Studio help:** Help in the top bar (or `?` when not typing) opens SWELL studio. Recipes run on the live board (week post, hook GIF, artwork still, motion layers, email letter). **Studio brief** (`shared/craftManual.ts`) is Isla's operator licence — laws, weekday playbooks, every house motion plate with a job, named stacks. Inspector click adds a motion layer; right-click a plate replaces that plate's look. Recipes never download PNG/GIF; you export yourself.
...
- **Motion** — 90 house presets in grouped strips (Atmosphere / Graphic devices / Structure / Occasional); empty TemplateStrip also offers Ledger Current. Atmosphere also has Vapor Drift, Static Shiver, Vignette Breathing, Horizon Shift, Carbon Weave, Parchment Heat, Liquid Quartz, Liquid Glass Shift, Thermal Heat Bloom, Dynamic Focal Vignette, Vellum Hysteresis, Gravitational Field, Breathing Monument, Vanishing Point Tunnel, Sundial Type. Graphic devices also have Ledger Fracture, Cybernetic Scanline, Ledger Margin Glow, Resin Gloss Sweep, Holographic Foil, Vellum Crease, Prismatic Focal Shift, Magnetic Edge Snap, Kinetic Stencil Punch, Guilloche Wave, Origami Unfold, Isometric Extrusion, Mechanical Escapement, Glowing Pill Pulse, Redact Highlight, Stroke Reveal Check, Telemetry Ghost, Reality Buffering, Slow Fax, Halftone Lamp. Structure also has Elastic Spring, Kinetic Squeeze, Crosshair Grid Track, Typewriter Cursor, Ledger Stitch, Kinetic Momentum Glide, Acoustic Waveform Pulse, Resonance Blur, Topographic Contour, Elastic Threading, Voronoi Partition, Liquid Text Mask, Odometer Roll, Dripping Text, Kinetic Word Press, Kinetic Assembly, Kintsugi Mend, Hourglass Countdown, Pendulum Letters. Occasional also has Ink Splash Bloom, Quantum Glitch, Ink Ripple, Focus Pull, Magnetic Ripple, Kinetic Strobe Pulse, Anode Flicker Decay, Phosphor Burn-In, Quantum Entanglement, Viral Hook Drop, Cinematic Hook Slam, Word Vortex, Chromatic Misregister, Icon Weather System, Stillness Static, Declassified Text, Ferrofluid Pull, Murmuration, Tab Escape, Moss Bloom, Ink Pileup.

**Motion Widgets port** — all 24 generative widgets across both `motion-widgets` bundles (dependency-free custom elements, `motion-core.js` base class) are reimplemented natively as house presets rather than embedded as web components, so they stay canvas-2D, palette/brand-driven, and export-compatible like every other plate. Dripping Text keeps its bespoke liquid-physics engine (`lib/motionDrip.ts` + `lib/motionWidget.ts`, exposed via the Motion inspector's liquid-preset/shape/interact controls) because it needs a real particle sim; the other 23 are ported as single-look `drawXxx(args)` functions in `lib/motionRuntime.ts` — one signature motion mode each rather than every sub-mode the original widget exposed, following the same fixed-look convention as the other presets. They pick up word/hook text, palette, texture and speed from the existing generic schema fields, so no bespoke Inspector UI or new schema fields were needed. First batch: Word Engine → Kinetic Word Press, Word Tornado → Word Vortex, The Machine That Spells → Kinetic Assembly, Error Cathedral → Chromatic Misregister, Breathing Monument, Emotional Loading Bar → Stroke Reveal Check, Telemetry Ghost, Fake 3D Tunnel → Vanishing Point Tunnel, Icon Weather System, Reality Buffering. Second batch ("set two"): Stillness Static, Declassified Text, Kintsugi Type → Kintsugi Mend, Ferrofluid Type → Ferrofluid Pull, Slow Fax, Sundial Type, Halftone Lamp, Hourglass Countdown, Murmuration, Pendulum Letters — these traded a few stateful/real-world inputs (device tilt, page scroll position, a real target date, true N-body flocking/pendulum physics) for deterministic, time-driven equivalents, since Craft's motion nodes are pure functions of elapsed time and don't carry a generic "target date" field; Hourglass Countdown in particular loops a fixed demo span rather than counting down to a real date. Third batch ("attention grabbers"): Favicon Escape → Tab Escape, Absence Bloom → Moss Bloom, Selection Ink → Ink Pileup — these traded real host-page side effects (mutating `document.title`/favicon, tracking tab-visibility while away, listening for `window.getSelection()` anywhere on the page) for the same self-contained, deterministic-loop treatment: Tab Escape draws its own mock browser-tab chip in-canvas instead of touching the real one, Moss Bloom runs a fixed grow/hold/clear cycle instead of tracking real absence, and Ink Pileup drops a scripted word list on a timer instead of listening for real selections.
...
**Per-node fields:** name, opacity, shadow preset, motion preset (CSS tween on text/shape/image — Still/Fade/Slide/Pop/Pulse; hidden on MotionNode plates). Text: body textarea (commits on blur, syncs back to structured copy), 6 type-style presets (Eyebrow/Heading/Subhead/Body/Caption/Stat), font size, align, font family (system + doc-embedded custom fonts), weight, letter-spacing, uppercase, colour. Shape: fill colour. Image: object-fit, 14 frame-shape buttons, 16 "look" presets, brightness/contrast, tint/wash colour, replace-image. **MotionNode** (selected Motion section): word on plate, background texture (solid/scanlines/grid/dots/waves/noise/paper), 4 filament colours, physics sliders, trigger/blend/fps, capture still, Record GIF, Replay pour, Advanced JSON. Dripping Text also exposes liquid presets (honey/water/slime/wax/ink/gel), drop shape, drip motion, gravity/drift/viscosity/chaos, font, and pointer interact (ripple/attract/repel/stretch). All: X/Y/W/H.
...
**Weekday creative direction** — `composePost.ts` maps each weekday to a fixed frame/shadow/motion combination, plus `applyPlaybookStack` which drops the studio-brief motion plates onto the Media frame (inspector-add, not a template rebuild). Relaying copy does not duplicate plates.
...
- `composeSocialPost(post, brand, logo)` builds the house week file (`li-landscape` ×7), fills copy, applies logo, weekday creative direction, and Isla's playbook motion stack from `shared/craftManual.ts` (Monday horizon+slam, Tuesday stamp+leak, and the rest). Overlay plates receive Hook 1 / Hook 2 as Word on plate. Friday promotes DataTicker to the hero; Sunday hides the deck and CTA; Wednesday shows Voice and hides the overlapping Body. `generateStillsForWeek` hangs photography on the week file for Wednesday and Saturday only (Sunday never gets a still).
...
| **Isla Quinn — MKT-2, Marketing/Creative Director** | `shared/craftDirector.ts` | Deterministic (not a live LLM call) — turns a brief into title/hook/hook2/body/CTA/hashtags via `copyFromAmmo()`, appends a rotated job-first packager-identity line (e.g. "We build the file. We do not lend it.") if the brief doesn't already carry one, and picks a stock visual from a fixed 6-entry library by keyword heuristics. |
...
| POST | `/craft/scan` | Casey only — write Creative Ammo Briefs, empty the week queue. No copy, boards, or stills. |
