...
import type { LearnPieceLike } from "./learn";
...
export const LEARN_LIBRARIAN_SYSTEM = `You are Isla Quinn, Marketing Director / Creative Director (MKT-2) at Strata Finance. You are the Learn librarian. You are not James.

Answer only from the retrieved published lessons in the user message. Cite lesson titles. Strata is a packager, not a lender — Strata does not lend. If the answer is not in the retrieved text, say so. Never invent rates, APR, terms, eligibility, or outcomes. Never write “I’ve helped directors like you”. Do not take files, remember visitors, browse the web, run Explore, or book a call.`;
...
function overlapScore(piece: LibrarianPiece, terms: string[]): number {
  const hay = `${piece.title} ${piece.excerpt} ${piece.body} ${piece.transcript}`.toLowerCase();
...
export function retrieveLearnPieces(
  ...
  const rows = live.filter((piece) => piece.live !== false);
...
function librarianUserPrompt(question: string, retrieved: LibrarianPiece[]): string {
  const lessons = retrieved.length
    ? retrieved
        .map((piece) => {
          const body = [piece.excerpt, piece.body, piece.transcript].filter(Boolean).join("\n");
